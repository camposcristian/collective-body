...
class CarouselWidget extends Component {
  ...
  _getData() {
    ...
    var promises = [];
    var items = [];
    var storageRef = fire.storage().ref(`Carousels/SupplierCarousels/${this.props.feed}.json`);
    promises.push(storageRef.getDownloadURL().then(url => {
      fetch(url, { method: 'GET' })
        .then(response => response.json())
        .then(json => {
          json.map((item, index) => {
            var storageRef = fire.storage().ref(`Images/SupplierCarousels/${feedName}/${index + 1}.jpg`);
            promises.push(storageRef.getDownloadURL().then(url => {
              item.picture = url;
              items.push(item);
            }))
